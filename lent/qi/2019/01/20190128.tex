...
We say that a code $(\mathcal{E}_n,\mathcal{D}_n)$ (i.e. an encoding-decoding scheme) has rate $R$ if $\ceil{M}\approx 2^{nR}$. Thus $R =\frac{\log |M|}{n}$. A rate is \term{achievable} if there exists a code with that rate such that 
...
\input{lent/qi/2019/01/tikz/20190128_tikzbinaryerasure.tex}
