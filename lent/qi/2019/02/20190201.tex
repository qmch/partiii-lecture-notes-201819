...
\end{align}
One can then show that $\rho_A \geq 0$ (is positive semi-definite) and $\Tr \rho_A=0$, so $\rho_A$ is in fact a density matrix. We call it the \term{reduced density matrix} or a reduced state.
...
\end{gather}
These are the so-called \term{maximally entangled} states, i.e. bipartite pure states such that their reduced states are completely mixed,
\begin{equation}
