...
\begin{defn}
Given a vector space $V$ over a field $F$ ($=\RR,\CC$), an \term{inner product} $i$ is a symmetric bilinear map
$$i:V\times V \to F.$$
In particular, we say that $i$ is \term{non-degenerate} if for every $v\in V$ ($v\neq 0$), there is a $w\in V$ ($w\neq v$) such that
...
\begin{defn}
A Lie algebra is \term{semi-simple} if it has no abelian ideals. (This is a weaker condition than simple, clearly.)
\end{defn}
\begin{ex}
